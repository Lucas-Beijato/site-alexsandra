...
const GET: APIRoute = async (request) => {
  ...
  posts.forEach((canva) => {
    if (!canva.data.for_sale) {
      urls_gallery.push(
        `<url><loc>${request.url.origin}/galeria/${canva.data.slug}</loc><changefreq>weekly</changefreq><priority>0.8</priority></url>`,
      );
    }
    if (canva.data.for_sale) {
      urls_for_sale.push(
        `<url><loc>${request.url.origin}/a_venda/${canva.data.slug}</loc><changefreq>weekly</changefreq><priority>0.8</priority></url>`,
      );
    }
  });
